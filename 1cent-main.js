const firebaseConfig = {
  ...
  const coinRef = firebase.database().ref('centCount');
  ...
  function updateCoinDisplay() {
    ...
    <img src="1cent-omote.png" width="25px" alt="1セント" style="vertical-align: middle; display:inline-block;">×${coinCount}枚`;
  }
...
document.getElementById('set-probabilities').addEventListener('click', () => {
  ...
  document.getElementById('bet-button').addEventListener('click', () => {
    ...
    for (let i = 0; i < totalCoins; i++) {
      ...
      coin.style.backgroundImage = 'url("1cent-omote.png")';
    ...
    setTimeout(() => {
      ...
      coins.forEach(coin => {
        ...
        if (isSelectedSide) {
          if (selectedSide === '表') {
            ...
            coin.style.backgroundImage = 'url("1cent-ura.png")';
            tailsCount++;
          }
        } else {
          if (selectedSide === '表') {
      ...
      sendMessage(`${selectedSide}に${betAmount}枚BETして${userWin}枚獲得。1セント: ${coinCount}${netChangeText}枚`, "ゲームシステム");
